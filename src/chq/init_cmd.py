...
def init_cmd(host, port, user, password, secure, database):
    ...
    try:
        ...
        click.echo(f"Creating table {database}.chq_query_log...")
        client.command(_CREATE_TABLE.format(database=database))

        click.echo(f"Creating materialized view {database}.chq_query_log_mv...")
    ...
    click.echo()
    click.echo("Done! New queries will be archived automatically.")
    click.echo()
    click.echo("To backfill existing data:")
    click.echo(f"  INSERT INTO {database}.chq_query_log SELECT * FROM system.query_log")
    click.echo()
